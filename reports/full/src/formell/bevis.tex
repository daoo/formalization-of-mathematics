...
den implementerade algoritmen \C{toom_cook_rec} är definierad med en oberoende
...
27 Qed.
\end{lstlisting}
På rad \C{1} och \C{2} formuleras lemmat och på rad \C{3} anges att beviset
startar genom \C{Proof}. Rad \C{4} anger med \C{elim=>} att beviset kommer
göras med induktion över den variabel som står först i
lemmat, \C{n}. Basfallet är trivialt precis som i prop \ref{prop:1} eftersom
\C{toom_cook_rec} är vanlig multiplikation när \C{n}$=0$ och delmålet det utgör
avslutas direkt med hjälp av \C{//}. Till höger om \C{|} ges instruktioner
till induktionssteget. Variabeln \C{n'} där \C{n' + 1}  = \C{n},
för det \C{n} som induktionen görs över instansieras.
Induktionsantagnandet \C{IHn}, polynomen \C{p} och \C{q} och antagandet
\C{V_inver} om att interpolationsmatrisen är inverterbar instansieras
också. Till sist skriver \C{/=} om målet genom att utveckla definitionen
av \C{toom_cook_rec}.

Raderna \C{5} till \C{7} sätter kortare beteckningar på några av uttrycken
...
Förs tar vi hand om \C{if ... then ... else} - uttrycket i vänsterledet.
...
gjort över graden av p och q, som i prop \ref{prop:1}.

Nu görs i stället på
rad \C{8} med \C{ifP} en falluppdelning över \C{if ... then ... else}-uttrycket
...
och löses liksom det triviala målet på rad \C{4} direkt med \C{//}.
...
med taktiken \C{have} på rad \C{9} där vi bara arbetar med denna del av
vänsterledet. Där använder vi på rad \C{12} lemmat \C{colP} som låter oss visa
...
rad \C{13} kan vi sedan slutligen visa att uttrycket i vänsterledet i
...
och \C{toom_cook_interpol}, som används på rad \C{14} respektive rad \C{15}
till \C{20} för att skriva om målet till
\begin{lstlisting}
 recompose b
...
\end{lstlisting}
Det görs på rad \C{20} genom lemmat \C{size_split_mul}. Detta är ytterligare
...
u.['X^b] * v.['X^b] = p * q
\end{lstlisting}
Då kan vi på rad \C{23} använda lemmat \C{recompose_split}, som ungefär
motsvarar lemma~\ref{lemma:2} i det informella beviset, för att visa att
``sammansättningen'' att de splittade polynomen genom att evaluera i $x^b$ är
